...
class Interpolated:
	...
	def evalDevData(self,trainObj,fileName):
		...
		f=open(fileName,"a+")
		f.write("Total:	"+str(len(self.wordTag))+"\n")
		f.write("Total UNK words:	  " +str(totalUnkWords)+"\n")
		f.write("Total Errors:	"+str(totalErrors)+"\n")
		f.write("Correct:	"+str(correct)+"\n")
		f.write("Accuracy:	"+str(correct/len(self.wordTag))+"\n")
		f.close()
...
class BaseLine:
	...
	def evalDevData(self,testFile,vocab,fileName):
		correct=0
		testOnly=0
		...
		for rows in lines:
			...
			if word in vocab:
				...
				testOnly+=1
		f=open(fileName,"a+")
		f.write("Total:  "+str(len(lines))+"\n")
		f.write("Total UNK words:  "+str(testOnly)+"\n")
		f.write("Correct:  "+str(correct)+"\n")
		f.write("Over all accuracy:  "+str(correct/(len(lines)))+"\n")
		f.close()
		return((actualTags,matchedTags,proposedTags,tagSet))

def printEvalMatrics(actual,matched,proposed,tags,fileName,bl=False):
...
def main():
	...
	for i in range(len(lang)):
		#Baseline
		f= open(lang[i]+"BaseLine.log","w+")
		f.write("Running base line on "+lang[i]+"\n")
		f.close()
		train=BaseLine(lang[i])
		vocab=train.getFrequent(trainFiles[i])
		actual,matched,proposed,tags=train.evalDevData(testFiles[i],vocab,lang[i]+"BaseLine.log")
		printEvalMatrics(actual,matched,proposed,tags,lang[i]+"BaseLine.log",True)
		#Raw 5 grams
		f= open(lang[i]+"raw.log","w+")
		f.write("Running Raw 5 gram classifier on "+lang[i]+"\n")
		f.close()
		train=RawFGrams(trainFiles[i])
		test=RawFGrams(testFiles[i])
		actual,matched,proposed,tags=test.evalDevData(train,lang[i]+"raw.log")
		printEvalMatrics(actual,matched,proposed,tags,lang[i]+"raw.log")
		# Absolute Discounting
		f= open(lang[i]+"interpolated.log","w+")
		f.write("Running Interpolated 5 gram classifier on "+lang[i]+"\n")
		f.close()
		train=Interpolated(trainFiles[i])
		test=Interpolated(testFiles[i])
		actual,matched,proposed,tags=test.evalDevData(train,lang[i]+"interpolated.log")
		printEvalMatrics(actual,matched,proposed,tags,lang[i]+"interpolated.log")
		# if i<len(lang)-1:
